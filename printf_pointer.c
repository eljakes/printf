 ...
 * @flags: The flags to apply (+, space, #)
 */
void print_pointer(va_list args, int *chars_printed, char *flags)
{
...
int _printf(const char *format, ...)
{
    ...
    while (*format)
    {
        ...
        {
            format++;
            char flags[] = "+# "; // Flags to check for (+, space, #)
            while (strchr(flags, *format))
            {
                if (*format == '+')
                {
                    // Handle '+' flag (force a sign)
                    // Add your code here
                }
                else if (*format == ' ')
                {
                    // Handle ' ' flag (add a space if no sign)
                    // Add your code here
                }
                else if (*format == '#')
                {
...
{
print_pointer(args, &chars_printed, flags);
}
}
